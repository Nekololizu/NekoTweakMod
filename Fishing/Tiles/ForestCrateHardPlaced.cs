...
{
    public class ForestCrateHardPlaced : ModTile // Here we are setting up everything for the tile while the item is placed
    {
        public override void SetDefaults()
        {
            ...
            TileObjectData.newTile.CoordinateHeights = new[] { 16, 18 }; // absoluteAquarian: each number is how tall each "ROW OF TILES" in the "SPRITESHEET" is
            TileObjectData.addTile(Type);
            ModTranslation name = CreateMapEntryName(); // creates a "name" for the modded tile
            name.SetDefault("Sakura Crate"); // Sets the item name to "Sakura Crate"
        ...
        {
            Item.NewItem(i * 16, j * 16, 32, 16, ItemType<Fishing.Items.ForestCrateHard>()); // where the class is located in the namespace, where file is located in the mod folders~
        }
    }
}
