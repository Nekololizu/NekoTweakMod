    ...
    public class ModifyAccessorie : GlobalItem
    {
        public override void UpdateAccessory(Item item, Player player, bool hideVisual)
        {
            if (item.type == ItemID.FeralClaws)
            {
                ...
                if (player.HeldItem.melee || player.HeldItem.summon)
                // if the player is holding/using either a melee or summoner weapon 
                // Thanks to Oli for telling me about player.helditem
                {
                    player.releaseUseItem = true;
                    // Makes the accessory give autoReuse/AutoSwing to the players item/weapon
                }
            }
            {
